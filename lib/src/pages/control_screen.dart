...
class _ControlScreenState extends State<ControlScreen>
  ...
  void initState() {
    ...
    // Al primer frame, auto–conectar A2DP
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _autoConnectA2dp();
      _resumeAuto = true;
    });
                ...
                return GestureDetector(
                  onTap: () {
                    if (bleConnected) {
                      ...
                      BluetoothHelper.disconnectBluetoothAudio().then((ok) {
                        print('🔊 A2DP clásico desconectado por botón: $ok');
                      });
                    } else {
                      // Plan A: conectar BLE
                      _controller.conectarManualBLE(context).then((okBle) {
                        if (okBle && widget.connectedDevice != null) {
                          ...
                          BluetoothHelper.connectBluetoothAudio().then((okClassic) {
                            print('🔊 A2DP clásico conectado por botón: $okClassic');
